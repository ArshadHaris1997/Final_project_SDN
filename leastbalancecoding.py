...
"""
    empirically get a time for flow static request.
    """
...
class iplb (object):
  ...
  def __init__ (self, connection, algorithm, service_ip, weights,servers):
    ...
    # Variables used in least_packet algorithm.
    self.least_packet_index = 0
    self.least_packet_pck_sent = 0
    ...
    for ip,expire_at in self.outstanding_probes.items():
      if t > expire_at:
        ...
        if ip in self.live_servers:
          ...
          del self.weights[ip]
          # Set the count of packet for least packet as 0.
          self.least_packet_pck_sent = 0
...
def least_packet_alg (balancer):
  """
  Select the next server for load balancing using the least_packet algorithm.
  """
  length = len(balancer.live_servers.keys())
  if balancer.least_packet_index >= length:
    balancer.least_packet_index = 0
  server_selected = list(balancer.live_servers.keys())[balancer.least_packet_index]
  balancer.least_packet_pck_sent = balancer.least_packet_pck_sent + 1
  
  if balancer.least_packet_pck_sent == balancer.weights[server_selected]:
    balancer.least_packet_index += 1
    balancer.least_packet_pck_sent = 0
